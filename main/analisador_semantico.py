...
class AnalisadorSemantico:
    ...
    def erro_sintatico(self, msg):
        raise Exception(f"Erro de Sintaxe na linha {self.token.linha}, coluna {self.token.coluna}: {msg} (token: {self.token})")
    ...
    def decl_continua(self, tipo, id_token):
        ...
        # Declaração de função
            ...
            # Inserir função na tabela de símbolos do escopo pai (global)
            ...
            if self.tipo_funcao_atual != 'void' and not self.return_encontrado:
                self.erro_semantico(f"Função '{self.escopo_atual}' do tipo {self.tipo_funcao_atual} deve ter um comando 'return'.")
    ...
    @rastrear("comando")
    def comando(self):
        # ... (código similar, mas chamando as novas funções com verificação)
        if self.token.tipo in {'INT', 'FLOAT', 'CHAR', 'BOOL'}:
            ...
            if prox and prox.tipo == 'ATRIB':
                ...
                self.chamada_funcao()
                self.match('DELIM', ';') # Consome o ';' após a chamada
            else:
                self.erro_sintatico("Esperado '=' ou '(' após identificador")
        elif self.token.tipo == 'IF': self.comando_if()
        elif self.token.tipo == 'WHILE': self.comando_while()
        elif self.token.tipo == 'RETURN': self.comando_return()
        # ... outros comandos
        else:
    ...
    def atribuicao(self):
        ...
        if tipo_variavel != tipo_expressao:
            self.erro_semantico(f"Não é possível atribuir um valor do tipo '{tipo_expressao}' a uma variável do tipo '{tipo_variavel}'.")
    ...
    def chamada_funcao(self):
        ...
        if len(tipos_argumentos) != len(params_esperados):
            self.erro_semantico(f"Função '{id_token.valor}' espera {len(params_esperados)} argumentos, mas recebeu {len(tipos_argumentos)}.")

        for i, arg_tipo in enumerate(tipos_argumentos):
            param_tipo = params_esperados[i]['tipo']
            if arg_tipo != param_tipo:
                self.erro_semantico(f"Argumento {i+1} da chamada da função '{id_token.valor}': esperado tipo '{param_tipo}', mas recebeu '{arg_tipo}'.")
    ...
    def comando_return(self):
        ...
        if self.tipo_funcao_atual == 'void':
            ...
            if self.token.tipo == 'DELIM' and self.token.valor == ';':
                self.erro_semantico(f"Função '{self.escopo_atual}' deve retornar um valor do tipo '{self.tipo_funcao_atual}'.")
            
            tipo_retorno = self.expressao()
            if tipo_retorno != self.tipo_funcao_atual:
                self.erro_semantico(f"Tipo de retorno incompatível. A função '{self.escopo_atual}' espera '{self.tipo_funcao_atual}' mas recebeu '{tipo_retorno}'.")

        self.match('DELIM', ';')
        self.return_encontrado = True
        
    # --- Funções de Expressão que retornam TIPOS ---

    @rastrear("expressao")
    def expressao(self):
        # Por simplicidade, vamos assumir que as expressões são aritméticas por enquanto
    ...
    def expressao_aritmetica(self):
        ...
        while self.token.tipo == 'OP_ARIT' and self.token.valor in ('+', '-'):
            ...
            # REQUISITO: Não permitir operações de tipos diferentes
            if tipo_esq != tipo_dir:
                self.erro_semantico(f"Operação aritmética entre tipos incompatíveis: '{tipo_esq}' e '{tipo_dir}'.")
            # O tipo resultante é o mesmo
        return tipo_esq
    ...
    def fator(self):
        if self.token.tipo == 'NUM_INT':
            ...
            self.match('STRING')
            return 'char' # Simplificação: string literal pode ser tratada como ponteiro de char
        elif self.token.tipo == 'TRUE' or self.token.tipo == 'FALSE':
            self.match(self.token.tipo)
            return 'bool'
        elif self.token.tipo == 'ID':
            # Verifica se é uma chamada de função ou uma variável
            prox = self.tokens[self.pos + 1] if self.pos + 1 < len(self.tokens) else None
            ...
            self.erro_sintatico("Fator inválido na expressão: esperado número, ID, ou '('")
    ...
    # Dummy methods para as partes não implementadas da gramática para evitar erros
    def comando_if(self): self.match('IF'); self.match('DELIM', '('); self.expressao(); self.match('DELIM', ')'); self.comando()
    def comando_while(self): self.match('WHILE'); self.match('DELIM', '('); self.expressao(); self.match('DELIM', ')'); self.comando()